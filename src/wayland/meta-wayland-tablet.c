                                           ...
                                           struct wl_resource *resource)
{
  ...
  wl_tablet_send_name (resource, clutter_input_device_get_device_name (device));

  if (sscanf (clutter_input_device_get_vendor_id (device), "%x", &vid) == 1 &&
      sscanf (clutter_input_device_get_product_id (device), "%x", &pid) == 1)
    wl_tablet_send_id (resource, vid, pid);

  /* FIXME: wl_tablet.type, wl_tablet.path missing */

  wl_tablet_send_done (resource);
}
